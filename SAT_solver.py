...
########################################### SIMPLIFY FUNCTION (used in sat) ########################################
    ...
    def simplify(self, literal):
        ...
        for clause_idx in self.literal_to_clause_dict[-literal]:
            ...
            if self.clause_dict[clause_idx].is_empty():
                return False
        try: # This try except is here because the literal occurence dict does not neccisarily have -literal as a key.
            self.literal_occurence_count_dict.pop(-literal)
        except:
            pass
    ...
    def add_and_copy(self, literal):
        ...
        new_clause_dict[0] = Clause([literal])  # 0 is reserved for the adding literal
        new_unit_clauses = {0}

        new_literal_to_clause_dict = deepcopy(self.literal_to_clause_dict) 
        new_literal_to_clause_dict[literal].append(0)
        
        new_literal_occurence_count_dict = deepcopy(self.literal_occurence_count_dict) 
...
if __name__ == '__main__':
    ...
    #zac = time.time()
    solution = sat(cnf, 'Jaro') ### the second Heuristic implementation is "RDLCS"
    #kon = time.time()
